...
'''
class stim(dict):
    def __init__(self, name, numconditions):
        self._dict = {}
        self._dict['Name'] = name
        self._dict['Data'] = np.zeros((numconditions,), dtype=stim_data_dtype)
...
class stim:
    ...
    def __init__(self, numconditions):
        self.stimname = np.zeros((1,), dtype=namestring_dtype)
        self.stimdata = np.zeros((numconditions,), dtype=stim_data_dtype)

    def print(self):
        print(self.stimname['Name'].value)
        for i in range(0,len(self.stimdata['Data']['start'])):
            print(self.stimdata['Data']['start'][i], self.stimdata['Data']['duration'][i], self.stimdata['Data']['value'][i])
        print
'''
...
class snirffile:
    ...
    def make_ml_array(self, nchans):
        return np.zeros((nchans,), dtype=ml_dtype)

    def make_stim_arrays(self, numconditions):
        the_stimname = np.zeros((1,), dtype=namestring_dtype)
        the_stimdata = np.zeros((numconditions,), dtype=stim_data_dtype)
        return the_stimname, the_stimdata

    def make_aux_arrays(self, ntimepoints):
        the_aux_tvec = np.zeros((ntimepoints,), dtype=np.float64)
        the_aux_dvec = np.zeros((ntimepoints,), dtype=np.float64)
        the_auxname = np.zeros((1,), dtype=namestring_dtype)
        return the_aux_tvec, the_aux_dvec, the_auxname

    def make_data_arrays(self, ntimepoints):
        the_data_tvec = np.zeros((ntimepoints,), dtype=np.float64)
        the_data_dvec = np.zeros((ntimepoints,), dtype=np.float64)
        the_ml = np.zeros((1,), dtype=ml_dtype)
        return the_data_tvec, the_data_dvec, the_ml

    def make_SD_arrays(self, numlambdas, numlambdaex, numsrc, numdet):
        the_SD_lambdas = np.zeros((numlambdas,), dtype=np.float64)
        the_SD_lambdaex = np.zeros((numlambdaex,), dtype=np.float64)
        the_SD_srcpos = np.zeros((numsrc,), dtype=pos_dtype)
        the_SD_detpos = np.zeros((numdet,), dtype=pos_dtype)
    ...
    def read_from_file(self):
        thehdf = h5py.File(self.filename, 'r')

        self.compliant = True
        self.fileversion = thehdf.attrs['format_version']
        if self.fileversion != 1.0:
            print("incorrect file version number")
            self.compliant = False

        # read back and construct the data struct
        self.data = readgroup(thehdf, '/data/')
        ...
        for dataset in self.data:
            dataattrs = dataset.attrs.items()
            for item in ['SourceIndex', 'DetectorIndex', 'WavelengthIndex', 'DataType', 'DataTypeIndex']:
                testval = dataset.attrs.get(item)
                if testval is None:
                    print("MEASUREMENT INVALID - required dataset attribute missing: ", item)
                    self.compliant = False
        ...
        # read back and construct the data struct
        self.stims = readgroup(thehdf, '/stim/')
        nelements = len(self.stims)
        if nelements > 0:
        ...
        # read back the aux
        self.auxs = readgroup(thehdf, '/aux/')
        nelements = len(self.auxs)
        if nelements > 0:
            print(nelements, " optional aux elements detected")

        # read the file attributes
        self.attrs = thehdf.attrs.items()
        print(self.attrs)

        # read the metadata
        metagroup = thehdf['/metadata/']
        self.metaattrs = metagroup.attrs.items()
...
def read_snirf(name):
    ...
    return thedata, theSD, thestims, theattrs, themetaattrs, theauxs, fileversion, compliant
